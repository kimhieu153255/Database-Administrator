...
CREATE OR REPLACE PROCEDURE REVOKE_PRIVS_TAB_USER_OR_ROLE(
    NAME IN VARCHAR2,
    PRIVS IN VARCHAR2,
...
BEGIN
    ...
    BEGIN
        IF WITH_GRANT_OPTION = 1 AND COUNT_ROLE != 0 THEN 
            RESULT := 'CANT REVOKE "WITH GRANT OPTION" TO ROLE';
        ELSIF WITH_GRANT_OPTION = 1 AND COUNT_ROLE = 0 THEN 
            EXECUTE IMMEDIATE ('REVOKE ' || PRIVS || ' ON SYSTEM.' || TAB_NAME || ' FROM ' || NAME||' CASCADE');
            RESULT := 'SUCCESS';
        ELSE  
            EXECUTE IMMEDIATE ('REVOKE ' || PRIVS || ' ON SYSTEM.' || TAB_NAME || ' FROM ' || NAME);
            RESULT := 'SUCCESS';
...
BEGIN
    ...
    BEGIN
        IF WITH_GRANT_OPTION = 1 THEN
            EXECUTE IMMEDIATE ('GRANT ' || PRIVS || ' ON SYSTEM.' || V_NAME || ' TO ' || NAME || ' WITH GRANT OPTION');
            RESULT := 'SUCCESS';
        ELSE
            EXECUTE IMMEDIATE ('GRANT ' || PRIVS || ' ON SYSTEM.' || V_NAME || ' TO ' || NAME);
            RESULT := 'SUCCESS';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    ...
    ELSIF COUNT_ROLE = 0 AND COUNT_USER = 0 THEN
        RESULT := 'OBJECT(ROLE-USER) NOT EXISTS';
    ELSE
        BEGIN
